...
Throughout this dissertation, we endeavor to define sensible mathematical notation that is globally consistent across chapters.
That being said, the notation in each chapter is self-contained, and the intended scope of each mathematical symbol is local to each chapter. 
For example, the variable $\mathbf{u}$ always means some kind of control vector, but the exact meaning of this variable changes slightly from chapter to chapter.
